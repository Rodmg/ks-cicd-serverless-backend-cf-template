...
if (config.log.logToFiles) {
  const logDir = path.join(
    process.env.HOME,
    ".ks-cicd-serverless-backend-cf-template-logs",
  );
  ...
  log.add(winston.transports.File, {
    ...
    filename: path.join(
      logDir,
      "ks-cicd-serverless-backend-cf-template-error.log",
    ),
    json: true,
  });
  ...
  log.add(winston.transports.File, {
    ...
    filename: path.join(
      logDir,
      "ks-cicd-serverless-backend-cf-template-warn.log",
    ),
    json: true,
  });
  ...
  log.add(winston.transports.File, {
    ...
    filename: path.join(
      logDir,
      "ks-cicd-serverless-backend-cf-template-debug.log",
    ),
    json: true,
  });
  ...
  requestLog.add(winston.transports.File, {
    ...
    filename: path.join(
      logDir,
      "ks-cicd-serverless-backend-cf-template-requests.log",
    ),
    json: true,
  });
}
